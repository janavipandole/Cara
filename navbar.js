...
${
  loggedIn
    ? `<a href="#" data-auth-action="logout" title="Logout" aria-label="Logout">
         <i class="ri-logout-box-r-line"></i>
       </a>`
    : `<a href="login.html" title="Sign In" aria-label="Login">
...
function updateAuthLink(link, loggedIn) {
  ...
  if (loggedIn) {
    ...
    if (icon) icon.className = 'ri-logout-box-r-line';
    ...
    if (icon) icon.className = 'ri-user-3-line';
    else link.textContent = 'Sign In';
  }
}
...
function initMobileNavbar() {
  if (mobileNavbarInitialized) return;
...
// Apply saved theme on load using data-theme
