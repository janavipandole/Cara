...
describe('applySharedCart', () => {
    ...
    beforeEach(() => {
        ...
        const match = appJs.match(/window\.applySharedCart = function \(action\) \{[\s\S]*?\n\};/);
    ...
    it('merges shared wardrobe items into the current cart state', () => {
        ...
        expect(cart[1]).toMatchObject({ name: 'Shared Tee', size: 'M', quantity: 2 });
        expect(sandbox.window.cachedCartState).toHaveLength(2);
        expect(sandbox.window.cachedCartState[1]).toMatchObject({ name: 'Shared Tee', size: 'M', quantity: 2 });
    });
});
