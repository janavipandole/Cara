...
/**
 * OutfitCompatibility — exportable class for unit testing.
 * Wraps the module-level COLOR_HARMONY / CLASHING_PAIRS logic.
 */
export class OutfitCompatibility {
  /**
   * Returns true when two colors are compatible (or either is missing).
   * @param {string|null|undefined} c1
   * @param {string|null|undefined} c2
   * @returns {boolean}
   */
  isColorCompatible(c1, c2) {
    if (!c1 || !c2) return true;
    const a = c1.toLowerCase();
    const b = c2.toLowerCase();
    if (a === b) return true;
    if (getColorClash(a, b)) return false;
    const harmony = COLOR_HARMONY[a];
    if (harmony && harmony.includes(b)) return true;
    const harmonyB = COLOR_HARMONY[b];
    if (harmonyB && harmonyB.includes(a)) return true;
    // Neutrals (white/black/grey/beige) are universally compatible
    const neutrals = ['white', 'black', 'grey', 'beige'];
    if (neutrals.includes(a) || neutrals.includes(b)) return true;
    return false;
  }

  /**
   * Returns up to 3 recommended fallback colors for a given color.
   * Falls back to ['white', 'black'] when color is unknown.
   * @param {string} color
   * @returns {string[]}
   */
  getRecommendedFallbacks(color) {
    const key = (color || '').toLowerCase();
    const palette = COLOR_HARMONY[key];
    if (palette) return palette.slice(0, 3);
    return ['white', 'black'];
  }
}
