...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const input =
    searchForm.querySelector("input[type='email']") ||
  ...
  dropdown.style.cssText =
    'display:none; position:absolute; background:#fff; border:1px solid #ccc; width:100%; z-index:9999; border-radius:4px; box-shadow:0 4px 8px rgba(0,0,0,0.1); padding:10px; max-height:200px; overflow-y:auto; color: #333;';
  ...
  const loadHistory = () => {
    const history =
      JSON.parse(localStorage.getItem('cara_search_history')) || [];
    dropdown.innerHTML = '';
    if (history.length === 0) {
      dropdown.innerHTML =
        "<p style='font-size:12px;color:#888;margin:0;'>No recent searches</p>";
      return;
    }

    const title = document.createElement('p');
    title.style.cssText =
      'font-size:11px; font-weight:700; color:#088178; margin:0 0 5px 0; text-transform:uppercase;';
