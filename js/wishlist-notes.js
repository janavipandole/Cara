...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const observer = new MutationObserver(() => {
    const rows =
      document.querySelectorAll('.wishlist-item-row') ||
    ...
    rows.forEach((row) => {
      ...
      wrapper.style.cssText =
        'margin-top: 10px; display: flex; flex-direction: column; gap: 6px; padding: 10px; background: rgba(0,0,0,0.03); border-radius: 6px;';

      // Get saved priority
      const savedPriority =
        localStorage.getItem(`cara_wishlist_priority_${itemId}`) || 'Medium';
      const savedNote =
        localStorage.getItem(`cara_wishlist_note_${itemId}`) || '';
      ...
      // Change listeners
      wrapper
        .querySelector('.wishlist-priority')
        .addEventListener('change', (e) => {
          localStorage.setItem(
            `cara_wishlist_priority_${itemId}`,
            e.target.value
          );
        });
  ...
  const targetContainer =
    document.getElementById('wishlist-container') || document.body;
