...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function getRecentlyViewed() {
    try {
      if (typeof localStorage !== 'undefined') {
        return safeParseList(window.safeGetItem(STORAGE_KEY));
      }
    } catch (e) {
  ...
  function saveRecentlyViewed(list) {
    try {
      if (typeof localStorage !== 'undefined') {
        window.safeSetItem(STORAGE_KEY, JSON.stringify(list));
        return true;
      }
    } catch (e) {
  ...
  function goToProduct(name) {
    try {
      window.safeSetItem('selectedProductId', name);
    } catch (e) {
  ...
  function resolveSelectedProductName() {
    try {
      var id = window.safeGetItem('selectedProductId');
      if (id) return id;
      var legacy = JSON.parse(window.safeGetItem('selectedProduct') || '{}');
