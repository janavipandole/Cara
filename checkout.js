...
window.updateCheckoutSummary = function () {
  ...
  // All financial calculations use integer cents (paise)
  ...
  const couponDiscountCents = Math.round(
    subtotalCents * couponPct / 100
  );

  // Check urgency discount
  const hasUrgency =
  ...
  const urgencyDiscount =
    hasUrgency
      ? subtotal *
        (window.CARA_CONFIG
          ? window.CARA_CONFIG.URGENCY_DISCOUNT_PCT
          : 0.05)
      : 0;

  // Check gift wrap
  const hasGiftWrap =
    document.getElementById('gift-wrap-opt')?.checked;
  ...
  const tax =
    subtotal *
    (window.CARA_CONFIG
      ? window.CARA_CONFIG.TAX_RATE
      : 0.18);

  // Check loyalty points discount
  const loyaltyPoints =
    parseInt(
      localStorage.getItem('cara_applied_loyalty_points'),
      10
    ) || 0;
  ...
  const loyaltyDiscount =
    loyaltyPoints /
    (window.CARA_CONFIG
      ? window.CARA_CONFIG.LOYALTY.POINTS_PER_RUPEE
      : 10);
  ...
  if (subtotalEl) {
    subtotalEl.textContent = formatCurrency(subtotalCents);
  }

  const taxEl = document.getElementById('summary-tax');
  if (taxEl) {
    taxEl.textContent = formatCurrency(taxCents);
  }
  ...
  if (couponCode) {
    if (!couponRow) {
      ...
      if (grandRow) {
        grandRow.parentNode.insertBefore(
          couponRow,
          grandRow
        );
      }
