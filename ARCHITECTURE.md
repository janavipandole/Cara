...
4. `global.css` / `bundle.css` — compatibility shims that re-export the canonical sheet (do not add new rules there)
...
3. **Defense in Depth**: Client-side HTML input sanitization, cookie-based auth, and server-side order pricing.
...
- **Backend (`backend/app`)**: FastAPI routers for auth, products, orders, payments, and admin analytics.
