...
|                          Security & Data Layer                        |
+-----------------------------------------------------------------------+
|  Input Shield & Sanitizer  |  SameSite Cookies  | Encrypted Local Storage |
+-----------------------------------------------------------------------+
                                   |
                                   v
+-----------------------------------------------------------------------+
...
3. **Defense in Depth**: Client-side HTML input sanitization, `SameSite=Lax` session cookies, and secure local storage encryption.
...
- **Security Middleware (`js/utils/sanitize.js`, `backend/app/api/auth.py`)**: Client-side HTML entity sanitization and server-side session cookies scoped with `SameSite=Lax` for CSRF protection.
