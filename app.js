...
// ── Price parser: strips ₹ $ , spaces and HTML entities ─────
...
// ── Dynamic toast (add/remove/coupon actions) ────────────────
//    Creates styled toast cards inside #toast-container.
//    window.showToast (for Share Wardrobe) is defined separately
//    in the WARDROBE SHARING section — do NOT remove that one.
...
function updateCartCount() {
  ...
  var desktopCount = document.getElementById('desktopCartCount');
...
// ── Show/hide empty-cart UI and the summary panel ────────────
function handleEmptyCartView() {
  var cart           = JSON.parse(localStorage.getItem('productsInCart')) || [];
  var emptyContainer = document.getElementById('empty-cart-container');
  var summaryColumn  = document.getElementById('cart-summary-column');
  var actionsBottom  = document.getElementById('cart-actions-bottom');
  ...
  if (cart.length === 0) {
    ...
    if (actionsBottom)  actionsBottom.style.display  = 'block';
  }
}
...
function addToCart(productName, productPrice, productImage, quantity, size) {
  ...
  var item = {
    ...
    image:    productImage,
    quantity: parsedQty,
    size:     String(size).replace('Size ', '')
  };
 
  var existing = cart.find(function (p) {
    return p.name === item.name && p.size === item.size;
  });
...
// ── handleAddToCart (called from singleProduct page) ────────
...
window.handleAddToCart = function () {
  ...
  addToCart(nameEl.innerText, priceEl.innerText, imgEl.src, qty, size);
  updateCartCount();
};
...
window.handleBuyNow = function () {
  ...
  if (!nameEl || !priceEl || !sizeEl || !qtyEl || !imgEl) return;
...
// ── loadCart — renders items AND updates the summary panel ───
window.loadCart = function () {
  var cart = JSON.parse(localStorage.getItem('productsInCart')) || [];
 
  // 1. Show/hide empty state + summary column
  handleEmptyCartView();
  ...
  // 2. Clear and re-render item rows
  itemsContainer.innerHTML = '';
  var subtotal = 0;
 
  cart.forEach(function (item, index) {
    var itemPrice    = parsePriceString(item.price);
...
'<div class="cart-item-actions" style="display: flex; gap: 8px;">' +
  ...
  //   'title="Save for Later" style="color: var(--text-secondary); background: none; border: none; font-size: 20px; cursor: pointer;">' +
  //     '<i class="ri-bookmark-line"></i>' +
  // '</button>' +
  '<button class="cart-item-remove" aria-label="Remove item" ' +
    'onclick="event.stopPropagation(); removeItem(' + index + ')" title="Remove">' +
...
'</div>';
  ...
  // 3. Update summary panel elements
  ...
  // Shipping: free above ₹3000, else ₹150
  var shipping = (subtotal > 0) ? (subtotal >= 3000 ? 0 : 150) : 0;
  ...
  if (window.appliedCoupon === 'WELCOME10' && subtotal > 0) discount = Math.round(subtotal * 0.10);
 
  // Grand total
  var grandTotal = Math.max(0, subtotal + tax + shipping - discount);
  ...
  if (shippingEl) {
    shippingEl.innerText = (shipping === 0 && subtotal > 0) ? 'FREE' : formatCurrency(shipping);
  ...
  // 4. Coupon input visual state
  var promoInput = document.getElementById('coupon-code');
  var promoBtn   = document.getElementById('apply-coupon-btn');
...
window.changeQuantity = function (index, change) {
  ...
  var newQty = (parseInt(cart[index].quantity) || 1) + change;
...
// ── removeItem ───────────────────────────────────────────────
//    Uses splice(index, 1) — removes ONE item, saves, re-renders.
window.removeItem = function (index) {
  var cart        = JSON.parse(localStorage.getItem('productsInCart')) || [];
  var removedName = cart[index] ? cart[index].name : 'Item';
  cart.splice(index, 1);
  localStorage.setItem('productsInCart', JSON.stringify(cart));
  loadCart();
  updateCartCount();
...
window.applyCoupon = function () {
  ...
  if (code === 'CARA20') {
    ...
    showToast('Invalid code. Try CARA20 or WELCOME10.', 'error');
  }
};
 
// ── Bootstrap on DOMContentLoaded ───────────────────────────
document.addEventListener('DOMContentLoaded', function () {
  // Coupon button delegation (works for dynamically rendered buttons too)
...
const totalDisplay = document.getElementById('summary-total');
  ...
  // SHIPPING
let shipping = subtotal > 0 ? (subtotal >= 3000 ? 0 : 150) : 0;
...
// UPDATE UI
if (subtotalDisplay) {
  subtotalDisplay.innerText = `₹${subtotal.toLocaleString('en-IN')}`;
}

if (taxDisplay) {
  taxDisplay.innerText = `₹${tax.toLocaleString('en-IN')}`;
}
...
(function () {
    // 1. Build and Inject Modal HTML Structure into document body
    const modalHTML = `
        <div class="quickview-modal" id="quickViewModal" role="dialog" aria-modal="true" aria-hidden="true">
            <div class="quickview-content">
                ...
                <div class="quickview-right">
                    ...
                    <div class="quickview-stars" id="qvModalStars">
