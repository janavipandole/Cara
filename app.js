...
// Function to toggle visibility of empty cart message
...
function addToCart(productName, productPrice, productImage, quantity, size) {
  ...
  let item = {
    ...
    image: productImage,
    quantity: parsedQty,
    size: size ? size.replace('Size ', '').replace('Size', '').trim() : 'M',
  };
...
(function () {
  ...
  function _showShopPage(pageNumber) {
  ...
  window._showShopPage = _showShopPage;
...
document.addEventListener('DOMContentLoaded', () => {
  const quizClose = document.querySelector('#quiz-modal .close');
  if (quizClose) {
...
function buyNow(productName, productPrice, productImage, quantity, size) {
...
window.shareWardrobe = function () {
  ...
  if (cart.length === 0) {
    showToast('Your cart is empty! Add some items before sharing.', 'warning');
    return;
  }
  try {
...
window.checkSharedWardrobe = checkSharedWardrobe;

function applySharedCart(action) {
...
window.applySharedCart = applySharedCart;
