...
window.loadCart = function () {
  ...
  const totals =
    typeof calculateCartTotals === 'function'
      ? calculateCartTotals(cart, window.appliedCoupon)
      : {
          subtotal: 0,
          shipping: 0,
          tax: 0,
  ...
  if (subtotalEl) {
    subtotalEl.innerText = formatCurrency(totals.subtotal);
  }
  ...
  if (taxEl) {
    taxEl.innerText = formatCurrency(totals.tax);
  }

  if (discountRow && discountEl) {
    if (totals.discount > 0) {
      discountRow.style.display = 'flex';
      discountEl.innerText = '-' + formatCurrency(totals.discount);
    } else {
  ...
  if (totalEl) {
    totalEl.innerText = formatCurrency(totals.grandTotal);
  }
...
function calculateCartTotals(cart, appliedCoupon) {
  let subtotal = 0;
  cart.forEach((item) => {
    const itemPrice = parsePriceString(item.price);
    const itemQty = parseInt(item.quantity) || 1;
  ...
  let discount = 0;
  if (appliedCoupon === 'CARA20' && subtotal > 0) {
    discount = Math.round(subtotal * 0.2);
  } else if (appliedCoupon === 'WELCOME10' && subtotal > 0) {
    discount = Math.round(subtotal * 0.1);
  } else if (appliedCoupon === 'FLAT50' && subtotal > 0) {
    discount = 50;
  }
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { calculateCartTotals, parsePriceString };
}
