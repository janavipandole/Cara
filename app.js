/* global updateCartTotal, loadCart, applyCoupon */
...
(function () {
  ...
  if (!paginationSection || !productSection) return;
  ...
  // Define Global Reacting State
  window.shopState = {
    allProducts: allProducts,
    activeFilteredProducts: [...allProducts],
    currentPage: 1,
  ...
  // Inject pagination button styles once
  if (!document.getElementById('pagination-button-styles')) {
    const style = document.createElement('style');
    style.id = 'pagination-button-styles';
    style.textContent = `
      #pagination button {
  ...
  window._renderShop = function renderShop() {
    const state = window.shopState;
    if (!state) return;

    // Hide all products from active DOM to avoid duplicates or leak
    ...
    const productsToShow = state.activeFilteredProducts.slice(
      startIndex,
      endIndex
    );

    state.firstContainer.innerHTML = '';
    state.firstContainer.style.display = 'flex';

    productsToShow.forEach((product) => {
      product.style.display = 'block';

      // Ensure injected product cards have alt text on images
      const img = product.querySelector('img');
      if (img && (!img.alt || img.alt.trim() === '')) {
        const title =
          product.querySelector('h5')?.textContent || 'Product Image';
        img.setAttribute('alt', title);
      }

      state.firstContainer.appendChild(product);
    });

    updatePaginationUI();
    handleNoResults(state.activeFilteredProducts.length);
  };

  function updatePaginationUI() {
    const state = window.shopState;
    if (!state || !state.paginationSection) return;

    // Set role and aria attributes on the pagination container
    state.paginationSection.setAttribute('role', 'navigation');
    state.paginationSection.setAttribute('aria-label', 'Shop pagination');
    state.paginationSection.innerHTML = '';

    const totalPages = Math.ceil(
      state.activeFilteredProducts.length / state.productsPerPage
    );
    ...
    prevArrow.setAttribute('aria-label', 'Go to previous page');
    if (activePage === 1) prevArrow.classList.add('disabled');
    prevArrow.addEventListener('click', () => {
      if (activePage > 1) {
        state.currentPage = activePage - 1;
        window._renderShop();
        state.productSection.scrollIntoView({
          behavior: 'smooth',
          block: 'start',
        });
      }
    });
    state.paginationSection.appendChild(prevArrow);
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      state.paginationSection.appendChild(pageLink);
    }
    ...
    nextArrow.setAttribute('aria-label', 'Go to next page');
    ...
    state.paginationSection.appendChild(nextArrow);
  }
    ...
    if (count === 0) {
      if (!noResultsMsg) {
        ...
        noResultsMsg.querySelector('p').textContent =
          `We couldn't find any products matching "${searchValue}". Please try a different search term or change your category filter.`;
        const container =
          document.getElementById('shop-container') || productSection;
  ...
  // Unified Coordinator
  window._performShopSearch = function performSearch() {
    const state = window.shopState;
    if (!state) return;
    ...
    const sortValue = sortMenu ? sortMenu.value : 'default';
    ...
    state.activeFilteredProducts = filtered;
    state.currentPage = 1;
    window._renderShop();
  };
  ...
  if (searchInput) {
    searchInput.addEventListener(
      'input',
      debounce(window._performShopSearch, 150)
    );
    searchInput.addEventListener('keyup', (e) => {
      if (e.key === 'Enter') window._performShopSearch();
    });
  }
  if (categoryFilter) {
    categoryFilter.addEventListener('change', window._performShopSearch);
  }
  if (sortMenu) {
    sortMenu.addEventListener('change', window._performShopSearch);
  }
  if (searchBtn) {
    searchBtn.addEventListener('click', window._performShopSearch);
  }
...
window.selectStyle = function (style) {
  ...
  if (categoryFilter) {
    ...
    for (let i = 0; i < categoryFilter.options.length; i++) {
      if (categoryFilter.options[i].value === style) {
        categoryFilter.value = style;
        hasOption = true;
        break;
      }
    }
  ...
  const state = window.shopState;
  if (state) {
    state.activeFilteredProducts = state.allProducts.filter((product) => {
      return product.getAttribute('data-category') === style;
    });
    state.currentPage = 1;
    if (typeof window._renderShop === 'function') window._renderShop();
  } else {
...
/* Unified search, filters, and sort are bound inside the unified system block. */
