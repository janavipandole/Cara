...
window.loadCart = function () {
  ...
  if (shippingDisplay) {
    shippingDisplay.innerText = shipping === 0 ? 'Free' : `₹${shipping}`;
  }
...
/* --- START: STYLE QUIZ POPUP FIXES & UI OVERRIDES --- */
(function() {
  // Override openQuiz to show with animation
  window.openQuiz = function () {
  ...
  // Override closeQuiz to hide with animation
  window.closeQuiz = function () {
    const modal = document.getElementById('quiz-modal');
    if (!modal) return;
    modal.classList.remove('active');
    // Wait for the transition (300ms) before changing display to none
    setTimeout(function() {
  ...
  // Override selectStyle to animate close, filter products, and show beautiful toast
  window.selectStyle = function (style) {
    closeQuiz();
    const products = document.querySelectorAll('.pro');
    products.forEach((product) => {
    ...
    if (typeof showToast === 'function') {
      showToast('Showing ' + style.charAt(0).toUpperCase() + style.slice(1) + ' style recommendations!', 'success');
    } else {
      alert('Showing ' + style + ' style recommendations!');
    }
  };

  // Dynamically inject beautiful, clean content to Style Quiz Modal on load
  function setupQuizModalContent() {
    const quizModal = document.getElementById('quiz-modal');
    if (!quizModal) return;

    // Set styling to none initially so it doesn't show up on load
    quizModal.style.display = 'none';

    // Clean inline styles and update content
    ...
    if (modalContent) {
      modalContent.innerHTML = `
        ...
        <p style="font-size: 16px; color: var(--text-secondary); margin-bottom: 25px; line-height: 1.6;">Select your preferred fashion vibe to curate your personalized style recommendations.</p>
        <div class="quiz-options">
          <button onclick="selectStyle('minimal')">✨ Minimal</button>
          <button onclick="selectStyle('street')">🔥 Street</button>
          <button onclick="selectStyle('formal')">💼 Formal</button>
        </div>
      `;
    }
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', setupQuizModalContent);
  } else {
    setupQuizModalContent();
  }
})();
/* --- END: STYLE QUIZ POPUP FIXES & UI OVERRIDES --- */
