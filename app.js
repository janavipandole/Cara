...
window.loadCart = function () {
    ...
    if (cartShippingEl) {
        cartShippingEl.innerText = shipping === 0 ? 'Free' : formatCurrency(shipping);
    }
    ...
    if (cartTotalEl) {
        ...
        if (strongEl) {
            strongEl.innerText = formatCurrency(grandTotal);
        } else {
            cartTotalEl.innerHTML = `<strong>${formatCurrency(grandTotal)}</strong>`;
        }
    }
