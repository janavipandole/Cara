(() => {
    ...
    if ('setAppBadge' in navigator) {
      if (totalItems > 0) {
        navigator
          ...
          .catch((err) => console.error('Error setting app badge:', err));
      } else if ('clearAppBadge' in navigator) {
        navigator
          .clearAppBadge()
          .catch((err) => console.error('Error clearing app badge:', err));
      }
    }
  }
  ...
  function addToCart(productName, productPrice, productImage, quantity, size) {
    return withCartLock(() => {
      ...
      if (isFirstItem && navigator.storage && navigator.storage.persist) {
        navigator.storage
          .persist()
          .then((granted) => {
            if (granted) {
              console.log('Persistent storage granted for cart data.');
            } else {
