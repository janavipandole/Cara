...
window.changeQuantity = function (index, change) {
  ...
  // force numbers
  cart[index].quantity = Number(cart[index].quantity);
...
window.loadCart = function () {
  ...
  if (shippingEl) {
    shippingEl.innerText = subtotal > 0 ? 'FREE' : '₹0';
  }
...
window.changeQuantity = function (index, change) {
  ...
  console.log("Before:", cart[index]);

  if (!cart[index]) return;

  const currentQty = parseInt(cart[index].quantity, 10);
  const newQty = currentQty + change;

  console.log("Current:", currentQty, "Change:", change, "New:", newQty);
